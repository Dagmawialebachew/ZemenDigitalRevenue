...
async def apply_migrations(dsn: str, directory: Path | None = None) -> list[str]:
    ...
    migrations_dir = directory or Path(__file__).with_name("migrations")
    ...
    try:
        ...
        existing_rows = await conn.fetch(
            "SELECT version, checksum FROM _zemen_schema_migrations ORDER BY version"
        )
        ...
        for migration in migrations:
            if migration.version in existing:
                if existing[migration.version] != migration.checksum:
                    raise RuntimeError(
                        f"Migration {migration.version} checksum changed after being applied"
                    )
                continue
            ...
            async with conn.transaction():
                ...
                await conn.execute(
                    """
                    INSERT INTO _zemen_schema_migrations (version, name, checksum)
                    VALUES ($1, $2, $3)
                    """,
                    migration.version,
                    migration.name,
